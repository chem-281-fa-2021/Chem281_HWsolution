	    ...
	    const unsigned int mrows, const unsigned int mcols)
{
  for (unsigned int i=0; i<mrows; i++)
    ...
    for (unsigned int j=0; j< mcols; j++)
      accum += matrix[mcols*i+j]*vector[j];
    result[i] = accum;
    }
}
		  ...
		  const unsigned int mrows, const unsigned int mcols)
{
  ...
  for (unsigned int i=0; i<mrows; i++)
  {
    for (unsigned int j=0; j<mcols; j++)
    {
      matrix[mcols*i+j] = (double) (rand()%10000); // bug 3: turn mrows to mcols
      //cout << matrix[mrows*i+j] << ' ';
    }
    //cout << endl;
  }
  for (unsigned int j=0; j<mcols; j++) 
      vector[j] = (double) (rand()%10000);
}
...
{
  const unsigned int mrows = 10;
  const unsigned int mcols = 20;
